...
image_filename_to_time = lambda x: int(x.split(".")[0][:3])


def add_to_mask_dict(masks_dict, mask_path):
    basename = os.path.basename(mask_path)
    cam = image_filename_to_cam(basename)
    time = image_filename_to_time(basename)
    mask = cv2.imread(mask_path)
    if time not in masks_dict:
        masks_dict[time] = [None] * 3  # FRONT_LEFT, FRONT, FRONT_RIGHT 1, 0, 2
    if cam == 1:
        masks_dict[time][0] = mask
    elif cam == 0:
        masks_dict[time][1] = mask
    elif cam == 2:
        masks_dict[time][2] = mask
...
def segment_with_text_prompt(datadir, BOX_TRESHOLD, TEXT_TRESHOLD, ignore_exists):
    ...
    for image_path in tqdm(image_files):
        ...
        print(
            f"detecting {boxes.shape[0]} boxed of sky in {image_path}, box_threshold: {box_threshold}, logits: {logits}"
        )
        if boxes.shape[0] != 0:
    ...
    for frame, masks in masks_dict.items():
        ...
        text = f"frame: {frame}"
        cv2.putText(
            merge_mask, text, (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 3, (255, 0, 0), 2
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--box_threshold", nargs="+", type=float, default=[0.3]
